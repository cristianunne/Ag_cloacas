...
class ClNodoTrSymbolEntity():
    ...
    def addNodoSymbol(self, tramo, type_simbol, ini_fin):
        ...
        idx_type_symbol = self.cl_nodo_tr_symbol.fieldNameIndex("ty_sym")
        idx_type_angulo = self.cl_nodo_tr_symbol.fieldNameIndex("angulo")
    ...
    def deleteNodoSymbol(self, tramo, option, layer_cl_tramos):
        ...
        if num_tramos_ref <= 1:
            ...
            idx_nodo_inicio =  layer_cl_tramos.fieldNameIndex("id_nodo_symbol_inicio")
            idx_nodo_final=  layer_cl_tramos.fieldNameIndex("id_nodo_symbol_final")
            
            attrs = {idx_nodo_inicio :  None, idx_nodo_final : None}
            
            layer_cl_tramos.startEditing()
            res = layer_cl_tramos.dataProvider().changeAttributeValues({ tramo.id() : attrs })
