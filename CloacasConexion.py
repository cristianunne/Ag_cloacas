...
from ClTrdomEntity import ClTrdomEntity
from Cl_InfotdEntity import Cl_InfotdEntity
...
class CloacasConexion():
    ...
    def __init__(self, iface, toolbar):
        ...
        # Create actions 
        self.nodos = QAction(QIcon(":/plugins/AGCloacas/conexion.png"),  QCoreApplication.translate("AG_Cloacas", "Crear Conexion Domiciliaria"),  self.iface.mainWindow())
    ...
    def conexionCloacas(self):
        ...
        if self.nodos.isChecked():
            ...
            try:
                #Verifico que las tablas esten cargadas
                layer_parcelas = QgsMapLayerRegistry.instance().mapLayersByName('parcelas')[0]
                layer_cl_tramos = QgsMapLayerRegistry.instance().mapLayersByName('cl_tramos')[0]
                layer_cl_infotd = QgsMapLayerRegistry.instance().mapLayersByName('cl_infotd')[0]
                layer_cl_trdom = QgsMapLayerRegistry.instance().mapLayersByName('cl_trdom')[0]
    ...
    def createPointCentroidsLayer(self, centroids, mem_layer):
        ...
        for i in range(num_cen):
            feature_cen[i].setGeometry(QgsGeometry.fromPoint(centroids[i]))
    ...
    def createConexion(self, feat_cltramos, layer_centroids, layer_cl_infotd, feat_parcela, layer_cl_trdom):
        ...
        poly = QgsGeometry.fromPolyline([point_aux, p2])
        ...
        if self.info_td.addPoint(p1, feat_parcela, feat_cltramos):
            ...
            self.iface.messageBar().pushMessage("Error", "Problema al crear las conexiones, verifique los datos e intente nuevamente", level=QgsMessageBar.CRITICAL)
    ...
    def evaluatedPosicionPoint(self, x_cen, y_cen, x_colseg, y_colseg):
        ...
        if(x_colseg < x_cen and y_colseg == y_cen):
            point_aux = QgsPoint(x_cen + 3.5, y_cen)
        ...
        #-------------------------------------------------------   
            ...
            point_aux = QgsPoint(x_cen, y_cen - 3.5)
        ...
        #-------------------------------------------------------  
            ...
            if (x_cen - x_colseg) > 2.5:
                ...
                point_aux = QgsPoint(x_cen + 2.5, y_cen + 2.5)
        ...
        #RESUELTO NO TOCARRRRR----------------------------------    
            ...
            if (x_cen - x_colseg) > 2.5:
                ...
                if (y_colseg - y_cen) > 2.5:
                    ...
                    x_ = x_cen - x_colseg
                    point_aux = QgsPoint(x_cen + x_, y_cen - 3.5)
                
                else:
                    y_ = y_colseg - y_cen
               
                    point_aux = QgsPoint(x_cen + 3.5, y_cen + y_)
        ...
        #----------------------------------
            ...
            if (x_colseg - x_cen) > 2.5:
                ...
                point_aux = QgsPoint(x_cen, y_cen + 2.5)
